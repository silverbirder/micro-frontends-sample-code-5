...
declare const AUTH0_CLIENT_SECRET: String;
declare const AUTH0_CALLBACK_URL: String;
declare const SALT: String;
declare const AUTH_STORE: any;
...
const validateToken = (token: { iss: any; aud: String; exp: number; iat: number; }) => {
    try {
...
const redirectUrl = (state: string | number | boolean) =>
...
export const handleRedirect = async (event: { request: { url: string; }; }) => {
...
const verify = async (event: { request: { headers: { get: (arg0: string) => any; }; }; }) => {
...
export const authorize = async (event: { request: { headers: { get: (arg0: string) => any; }; }; }) => {
...
export const logout = (event: { request: { headers: { get: (arg0: string) => any; }; }; }) => {
    const cookieHeader = event.request.headers.get('Cookie')
    if (cookieHeader && cookieHeader.includes(cookieKey)) {
        return {
            headers: {
